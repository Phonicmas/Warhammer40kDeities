...
namespace Deities40k
{
    public class Need_KhornateHunger : Need
    {
        ...
        public override void NeedInterval()
        {
            ...
            if (CurLevel > 0 && pawn.MentalStateDef == Deities40kDefOf.BEWH_KhornateHungerBeserk)
            {
                pawn.MentalState.RecoverFromState();
            }
            if (CurLevel == 0 && !pawn.InMentalState)
            {
                pawn.mindState.mentalStateHandler.TryStartMentalState(Deities40kDefOf.BEWH_KhornateHungerBeserk);
            }
        }
